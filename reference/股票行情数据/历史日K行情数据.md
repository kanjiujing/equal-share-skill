...
| `stockCodes` | `str` | 是 | - | 支持查询单只股票和多只股票（单次查询不超过20个），如果是多只，以逗号隔开；示例：'000001',，'600519' |
| `startDate` | `str` | 否 | - | 开始日期 startDate和endDate区间不能超过一年，如超过数据会截断，如果都为空，，则查询最新交易日，示例：2023-01-01 |
| `endDate` | `str` | 否 | - | 结束日期 startDate和endDate区间不能超过一年，如超过数据会截断，如果都为空，，则查询最新交易日，示例：2023-12-31 |
